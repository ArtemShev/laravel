    ...
    <div class="raw offset-2">
        ...
        <form method="post" action="{{ route('admin.users.update', ['user' => $user]) }}">
            ...
            <div class="form-group">
                ...
                <input type="password" class="form-control @if($errors->has('password')) alert-danger @endif" name="password" id="password" value="{{ $user->password }}">
                @error('password') <strong style="color: red">{{$message}}</strong>@enderror
            </div>
            <div class="form-group">
                <label for="is_admin">Админ</label>
                <input type="number" class="form-control @if($errors->has('is_admin')) alert-danger @endif" name="is_admin" id="is_admin" value="{{ $user->is_admin }}">
